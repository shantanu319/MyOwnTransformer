...
Original file is located at
    https://colab.research.google.com/drive/1ndBDn8mcskOBsxuzUPpi-Z-ab47_Wuys
"""
...
def data_feeder(data, batch_size, seq_len, device):
    ...
    for i in range(0, num_sequences, batch_size):
        x = data[i:i+batch_size] # data[i:i+batch_size] has shape [batch_size, seq_len]
        if x.size(0) < batch_size:
            break
        yield x[:, :-1], x[:, 1:] # input is x[:, :-1], target is x[:, 1:]
