...
The tokenizer (tokenizer.py) is a minbpe-style byte-level BPE with the GPT-2 pre-tokenization regex bolted on. It's stdlib-only — no tiktoken or sentencepiece — so the merge loop is transparent and hackable. prepare.py streams TinyStories from HuggingFace, trains BPE on the first N docs (10k by default), then re-tokenizes the full train split to train.bin and deterministically splits the validation split 50/50 into val.bin + test.bin by alternating document index. The .bin shards are raw uint16 token arrays separated by <|endoftext|>, which train.py mmaps for zero-copy batch sampling.
